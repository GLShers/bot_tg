...
async def com_start(message: Message, state: FSMContext):
    ...
    if is_profile_fully_filled:
        ...
        welcome_text = (
            ...
            f"<b>🎯 ВАШ ПЕРСОНАЛЬНЫЙ ЦЕНТР УПРАВЛЕНИЯ НЕЙРОКОММЕНТИНГОМ</b>\n\n"
            f"<b>📊 ДОСТУПНЫЕ ИНСТРУМЕНТЫ:</b>\n"
            f"• 👤 Настройка и оптимизация профиля\n"
            f"• ⭐ Аналитика и статистика выполнения\n"
            f"• 💎 Управление премиум-возможностями\n"
            f"• 🚀 Запуск и мониторинг активных сессий\n\n"
            f"<b>💡 РЕКОМЕНДАЦИЯ:</b>\n"
            f"🔍 <i>Проверьте все разделы меню и используйте инструменты для оптимизации и повышения эффективности.</i>\n\n"
            f"<b>🔗 СВЯЗАТЬСЯ С НАМИ:</b>\n"
            f"💬 <i>Если у Вас возникнут вопросы или потребуется помощь, обратитесь в нашу поддержку.</i>\n\n"
            f"<b>🌟 ЖЕЛАЕМ ВАМ УСПЕШНОЙ РАБОТЫ И ВЫСОКИХ РЕЗУЛЬТАТОВ!</b>"
        )
...
async def process_new_des_profile(message: Message, state: FSMContext):
    ...
    if len(new_des_profile) > 70:
        ...
        msg = await message.answer(
            "<b>🎭 Выберите персональный стиль AI-комментариев</b>\n\n"
            "Стиль комментариев напрямую влияет на конверсию и восприятие Вашего бренда.\n\n"
            "🔹 <b>Стиль Сета Година (Маркетинг и продажи)</b>\n"
            "🎯 Идеально для монетизации и продаж через личный бренд.\n"
            "✅ Краткость, чёткое предложение ценности, убедительные призывы к действию.\n\n"
            "🔹 <b>Стиль Гая Кавасаки (Привлечение внимания)</b>\n"
            "🎯 Оптимально для блогеров, инфлюенсеров, контент-креаторов.\n"
            "✅ Яркие эмоции, умеренная провокация, интригующие заявления, высокая вовлеченность.\n\n"
            "🔹 <b>Стиль Илона Маска (Экспертность и технологии)</b>\n"
            "🎯 Лучший выбор для технических, научных и экспертных каналов.\n"
            "✅ Лаконичность, глубина мысли, элементы футуризма, экспертная позиция.\n\n"
            "💡 <i>Выберите стиль, который максимально соответствует формату и цели Вашего канала.</i>",
            parse_mode="HTML",
...
async def compile_bot(callback: CallbackQuery, state: FSMContext):
    ...
    welcome_text = (
        f"<b>🏆 НЕЙРОКОММЕНТИНГ АКТИВИРОВАН!</b>\n\n"
        f"<b>🚀 ВАШ ЦЕНТР УПРАВЛЕНИЯ ИНТЕЛЛЕКТУАЛЬНОЙ СИСТЕМОЙ</b>\n\n"
        f"<b>📊 МГНОВЕННЫЙ ДОСТУП К ФУНКЦИЯМ:</b>\n"
        f"• 💹 Статистика активности и конверсии в реальном времени\n"
        ...
        f"<b>🔥 ПРЕИМУЩЕСТВА ВАШЕЙ СИСТЕМЫ:</b>\n"
...
async def handle_launch_bot(callback: CallbackQuery):
    ...
    if user.sub_id == 1:
        ...
        await callback.message.edit_text(
            ...
            "• Демонстрация AI-комментариев к вашему контенту\n"
            "• Оценка потенциала системы для вашего канала\n"
            "• Примеры комментариев, которые привлекают подписчиков\n\n"
            "<b>📊 КАК ЭТО РАБОТАЕТ:</b>\n"
            "1. Перешлите сюда любой пост из вашего канала\n"
            "2. AI проанализирует контент и создаст комментарий\n"
            "3. Вы увидите, как это будет работать в боевом режиме\n\n"
            "<b>⚠️ ОГРАНИЧЕНИЯ БЕСПЛАТНОЙ ВЕРСИИ:</b>\n"
            "• Комментарии не публикуются автоматически\n"
            "• Работает только с пересланными постами\n"
            "• Доступно только 3 тестовых генерации\n\n"
