...
from aiogram.types import Message, CallbackQuery, BotCommand, BotCommandScopeDefault, InlineKeyboardButton, InlineKeyboardMarkup
...
from app.utils.channel_checker import check_channels
...
async def handle_add_channels_callback(callback: CallbackQuery, state: FSMContext):
    ...
    # Поскольку пользователь нажал "Добавить каналы" в меню редактирования профиля,
...
async def com_start(callback: CallbackQuery):
    ...
    if not images:
        await callback.message.answer(
            ...
            "Мы собираем новые отзывы от клиентов. Вы можете стать первым, кто оставит отзыв и получить +14 дней к подписке!",
            parse_mode="HTML",
...
async def edit_link(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f"Формат: t.me/название_канала (рекомендуем)\n\n"
    ...
    if user.sub_id == 1:
        ...
        if not channels:
            ...
            await callback.message.edit_text(
                channels_text,
                parse_mode="HTML",
                reply_markup=kb.check_channels_keyboard()
            )
